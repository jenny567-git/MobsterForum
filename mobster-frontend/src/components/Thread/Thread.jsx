  ...
  return (
    <div className="thread-container" onClick={() => navigate(`/thread/${thread.threadId}`)}>
      <div className="thread-title">
        {thread.author.isActive && < div className="thread-metadata"><span>Posted by <p>{thread.author.userName}</p> in <p>{thread.familyName}</p> at {thread.createdAt}</span></div>} 
        {!thread.author.isActive && < div className="thread-metadata"><span>Posted by <p>&#91;Deceased&#93;</p> in <p>{thread.familyName}</p> at {thread.createdAt}</span></div>} 
